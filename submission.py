...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState) -> str:
        ...
        initial_action = Directions.STOP
        initial_depth = 0
        value = float('-inf')
        for action in gameState.getLegalActions(self.index):
          temp_value = value
          value = self.__minimax(gameState.generateSuccessor(self.index, action), self.index + 1, initial_depth)
          if value > temp_value:
            initial_action = action
        return initial_action

    def __minimax(self, gameState: GameState, index, depth):
      ...
      if index == 0:
        ...
        for action in gameState.getLegalActions(index):
            value = max(value, self.__minimax(gameState.generateSuccessor(index, action), index + 1, depth))

      else:
        value = float('inf')
        if index + 1 == gameState.getNumAgents():
            for action in gameState.getLegalActions(index):
                value = min(value, self.__minimax(gameState.generateSuccessor(index, action), index, depth + 1))
        else:
            for action in gameState.getLegalActions(index):
                value = min(value, self.__minimax(gameState.generateSuccessor(index, action), index + 1, depth))
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    # BEGIN_YOUR_CODE (our solution is 0 lines of code, but don't worry if you deviate from this)
      raise Exception("Not implemented yet")
    # END_YOUR_CODE
